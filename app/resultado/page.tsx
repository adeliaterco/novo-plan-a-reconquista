...
// ✅ CORREÇÃO CRÍTICA: Função UTM blindada contra corrupção
...
export default function ResultPageFixed() {
  // ✅ CORREÇÃO CRÍTICA: Verificação de hidratação PRIMEIRO
  ...
  // ✅ CORREÇÃO CRÍTICA: useEffect de hidratação PRIMEIRO
  useEffect(() => {
    setIsMounted(true)
    setIsClient(typeof window !== 'undefined')
  }, [])

  // ✅ CORREÇÃO CRÍTICA: Renderização condicional IMEDIATA
  ...
  // ✅ PERSONALIZAÇÃO BASEADA NO QUIZ - MOVIDO PARA CÁ
  useEffect(() => {
  ...
  }, [isClient, isMounted, currentRevelation, showVSL, showOffer, showFinalCTA])
  ...
  useEffect(() => {
    if (!showVSL || !isClient || !isMounted || !videoContainerRef.current) return
  ...
  }, [showVSL, isClient, isMounted, userGender])
  ...
  // ✅ CORREÇÃO: Função de compra blindada
  ...
  }, [currentRevelation, userGender, getPersonalizedSituation, isClient, showVSL, showOffer, showFinalCTA])
